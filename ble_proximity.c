...
void ble_proximity_handler() {
  if (ble_proximity_handle->timer_tick) {
      ble_proximity_handle->timer_tick = false;
      if (ble_proximity_handle->module.status > BLE_PROXIMITY_MODULE_STATUS_NOT_INIT) {
...
void ble_proximity_send_state(void) {
  ...
  ble_proximity_uart_write_data(BLE_PROXIMITY_CMD_MODULE_STATUS, data, sizeof(data));
}
...
void ble_proximity_send_uss_distance(float distance) {
  ...
  ble_proximity_uart_write_data(BLE_PROXIMITY_CMD_DISTANCE_DATA, data, sizeof(data));
}
...
void ble_proximity_uart_write_data(ble_proximity_cmd_t cmd, uint8_t *data, uint8_t data_len)
{
    if (!ble_proximity_chk_queue_full(&ble_proximity_handle->uart.tx_queue)) {
        ...
        tx_packet.crc = ble_proximity_calc_crc(tx_data, BLE_PROXIMITY_UART_PACK_SIZE - 2); // CRC8
...
bool ble_proximity_uart_read_finish(unsigned int channel, unsigned int sequenceNo, void *userParam)
{
  ...
  if (ble_proximity_handle->uart.rx_buff[BLE_PROXIMITY_UART_PACK_POS_PREAMBLE] == BLE_PROXIMITY_UART_PREAMBLE) {
      if (ble_proximity_handle->uart.rx_buff[BLE_PROXIMITY_UART_PACK_POS_CRC] == ble_proximity_calc_crc(ble_proximity_handle->uart.rx_buff, BLE_PROXIMITY_UART_PACK_SIZE - 2)) {
          if (!ble_proximity_chk_queue_full(&ble_proximity_handle->uart.rx_queue)) {
              ...
              ble_proximity_add_in_queue(&ble_proximity_handle->uart.rx_queue, rx_packet);
...
bool ble_proximity_uart_write_finish(unsigned int channel, unsigned int sequenceNo, void *userParam)
{
  ...
  ble_proximity_handle->uart.tx_queue.pack[ble_proximity_handle->uart.tx_current_pack].status = false;
  ...
  for (int i = 0; i < ble_proximity_handle->uart.tx_queue.size; i++) {
      if (ble_proximity_handle->uart.tx_queue.pack[index].status) {
          if (ble_proximity_handle->uart.tx_active != true) {
              ...
              ble_proximity_handle->uart.tx_buff[BLE_PROXIMITY_UART_PACK_POS_PREAMBLE] = BLE_PROXIMITY_UART_PREAMBLE;
              ble_proximity_handle->uart.tx_buff[BLE_PROXIMITY_UART_PACK_POS_CMD] = ble_proximity_handle->uart.tx_queue.pack[index].cmd;
              ...
              ble_proximity_handle->uart.tx_buff[BLE_PROXIMITY_UART_PACK_POS_CRC] = ble_proximity_handle->uart.tx_queue.pack[index].crc;
          }
      }
      index = (index + 1) % BLE_PROXIMITY_UART_QUEUE_SIZE;
  }
...
bool ble_proximity_add_in_queue(ble_proximity_uart_queue_t *queue, ble_proximity_uart_pack_t pack)
{
...
bool ble_proximity_del_from_queue(ble_proximity_uart_queue_t *queue, ble_proximity_uart_pack_t *pack)
{
