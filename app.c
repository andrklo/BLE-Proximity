...
void gpio_init(void)
{
  ...
  GPIO_PinModeSet(USS_TRIGGER_PORT, USS_TRIGGER_PIN, gpioModePushPull, 1);
  GPIO_PinModeSet(USS_ECHO_PORT, USS_ECHO_PIN, gpioModeInputPull, 1);
  GPIO_PinModeSet(IR_PORT, IR_PIN, gpioModeInputPull, 1);

  // Включаем прерывание на Echo пине
  GPIO_ExtIntConfig(USS_ECHO_PORT, USS_ECHO_PIN, USS_ECHO_PIN, true, true, true);
  // Регистрация callback-функции для Echo
  GPIOINT_CallbackRegister(USS_ECHO_PIN, gpio_callback);

  // Включаем прерывание на Echo пине
  GPIO_ExtIntConfig(IR_PORT, IR_PIN, IR_PIN, true, true, true);
  GPIOINT_CallbackRegister(IR_PIN, gpio_callback);
}
...
void gpio_callback(uint8_t pin)
{
  if (ble_proximity_get_state() > BLE_PROXIMITY_MODULE_STATUS_NOT_INIT) {
      ...
      if (pin == IR_PIN) {
          ...
          ble_proximity_set_ir_state((bool)!GPIO_PinInGet(IR_PORT, IR_PIN));
      }
  }
}
...
void uart_init_dma(void)
{
  ...
  // Запросить канал DMA
  DMADRV_AllocateChannel(&uart_dma_channel_rx, NULL);
  DMADRV_AllocateChannel(&uart_dma_channel_tx, NULL);
}

void uart_dma_send(void)
{
  // Передача данных через DMA
  DMADRV_MemoryPeripheral(uart_dma_channel_tx, dmadrvPeripheralSignal_USART0_TXBL,
                          (void *)&(USART0->TXDATA),
                          ...
                          ble_proximity_uart_write_finish, NULL); // Коллбек (необязательно)
}

void uart_dma_receive(void)
{
  // Ожидание приема данных через DMA с callback-функцией
  DMADRV_PeripheralMemory(uart_dma_channel_rx, dmadrvPeripheralSignal_USART0_RXDATAV,
...
void timer_periodic_init(void)
{
  ...
  sl_status_t status = app_timer_start(&periodic_timer, TIMER_PERIODIC_PERIOD, timer_periodic_callback, NULL, true);
...
{
    GPIO_PinOutSet(USS_TRIGGER_PORT, USS_TRIGGER_PIN);

    // Запускаем таймер для отключения Trigger через 10 микросекунд
    sl_status_t status = app_timer_start(&trigger_pulse_timer, TIMER_TRIGGER_PERIOD, timer_trigger_pulse_callback, NULL, false);
    if (status != SL_STATUS_OK) {
        // Обработка ошибки
    }
}
...
void timer_trigger_pulse_callback(app_timer_t *timer, void *data)
{
  (void)data;
  (void)timer;
  GPIO_PinOutClear(USS_TRIGGER_PORT, USS_TRIGGER_PIN);
}
...
void handle_device_states(void)
{
  ...
  if (ble_proximity_uart_get_req(BLE_PROXIMITY_UART_DEST_TX)) {
      ble_proximity_uart_set_req(BLE_PROXIMITY_UART_DEST_TX, false);
      uart_dma_send();
  }
  if (!ble_proximity_uart_get_state(BLE_PROXIMITY_UART_DEST_RX)) {
      uart_dma_receive();
      ble_proximity_uart_set_state(BLE_PROXIMITY_UART_DEST_RX, true);
  }
}
...
sl_status_t uss_distance_update_characteristic(void)
{
  ...
  uint8_t frac_part = (uint8_t)((distance - int_part) * pow(10, 2));  // Дробная часть
